...
void OdomEstimationClass::updatePointsToMap(const pcl::PointCloud<pcl::PointXYZI>::Ptr &edge_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &surf_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &edge_match_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &edge_map_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &surf_matche_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &surf_map_in, bool mode)
{
    ...
    if (laserCloudCornerMap->points.size() > 10 && laserCloudSurfMap->points.size() > 50)
    {
        ...
        for (int iterCount = 0; iterCount < optimization_count; iterCount++)
        {
            ...
            // addEdgeCostFactor(downsampledEdgeCloud, laserCloudCornerMap, problem, loss_function);
            // addMatchedEdgeCostFactor(edge_match_in, edge_map_in, laserCloudCornerMap, problem, loss_function);
            if (mode)
            {
                // if (edge_match_in->size() > 7)
                    // addMatchedSurfCostFactor(edge_match_in, edge_map_in, problem, loss_function);
                if (edge_match_in->size() > 0)
                    addMatchedPointCostFactor(edge_match_in, edge_map_in, problem, loss_function);
                else
                    addEdgeCostFactor(downsampledEdgeCloud, laserCloudCornerMap, problem, loss_function);
            }
            // if (surf_matche_in->size() > 30)
            // addMatchedSurfCostFactor(surf_matche_in, surf_map_in, problem, loss_function);
...
void OdomEstimationClass::addMatchedEdgeCostFactor(const pcl::PointCloud<pcl::PointXYZI>::Ptr &pc_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &matched_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &map_in, ceres::Problem &problem, ceres::LossFunction *loss_function)
{
    int corner_num = 0;
    for (int i = 0; i < (int)pc_in->points.size(); i++)
    {
        ...
        // if (pointSearchSqDis[4] < 1.0)
        {
            ...
            for (int j = 0; j < 5; j++)
            {
                Eigen::Vector3d tmp(map_in->points[i].x,
                                    map_in->points[i].y,
                                    map_in->points[i].z);
                center = center + tmp;
    ...
    if (corner_num < 20)
    {
        fprintf(stderr, "total: %d not enough correct matched edge points: %d \n", (int)pc_in->points.size(), corner_num);
    }
}
...
void OdomEstimationClass::addMatchedPointCostFactor(const pcl::PointCloud<pcl::PointXYZI>::Ptr &pc_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &map_in, ceres::Problem &problem, ceres::LossFunction *loss_function)
{
    int surf_num = 0;
    for (int i = 0; i < (int)pc_in->points.size(); i++)
    {
        ...
        double dx = pc_in->points[i].x - map_in->points[i].x;
        double dy = pc_in->points[i].y - map_in->points[i].y;
        double dz = pc_in->points[i].z - map_in->points[i].z;
        if(sqrt(dx*dx + dy*dy + dz*dz) < 1.5){
            Eigen::Vector3d curr_point(pc_in->points[i].x, pc_in->points[i].y, pc_in->points[i].z);
            Eigen::Vector3d map_point(map_in->points[i].x, map_in->points[i].y, map_in->points[i].z);

            ceres::CostFunction *cost_function = new PointToPointCostFunction(curr_point, map_point);
            problem.AddResidualBlock(cost_function, loss_function, parameters);
    ...
    if (surf_num < 20)
    {
        fprintf(stderr, "point total: %d not enough correct matched surf points: %d \n", (int)pc_in->points.size(), surf_num);
    }
}
void OdomEstimationClass::addMatchedSurfCostFactor(const pcl::PointCloud<pcl::PointXYZI>::Ptr &pc_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &map_in, ceres::Problem &problem, ceres::LossFunction *loss_function)
{
    int surf_num = 0;
    for (int i = 0; i < (int)pc_in->points.size(); i++)
    {
        ...
        // if (pointSearchSqDis[4] < 1.0)
        {
            ...
            for (int j = 0; j < 5; j++)
            {
                matA0(j, 0) = map_in->points[i].x;
                matA0(j, 1) = map_in->points[i].y;
                matA0(j, 2) = map_in->points[i].z; // map_in->points[i].z
            }
            ...
            for (int j = 0; j < 5; j++)
            {
                ...
                if (fabs(norm(0) * map_in->points[i].x +
                         norm(1) * map_in->points[i].y +
                         norm(2) * map_in->points[i].z + negative_OA_dot_norm) > 0.2)
                {
                    planeValid = false;
                    break;
                }
    ...
    if (surf_num < 20)
    {
        fprintf(stderr, "total: %d not enough correct matched surf points: %d \n", (int)pc_in->points.size(), surf_num);
    }
}
