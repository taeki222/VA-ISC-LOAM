...
void odomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
    ...
    FILE *fp = NULL;
    fp = fopen(Odo_RESULT_PATH.c_str(), "a");

    Eigen::Quaterniond result_q;
    Eigen::Vector3d result_t;

    result_q.w() = laserOdometry.pose.pose.orientation.w;
    result_q.x() = laserOdometry.pose.pose.orientation.x;
    result_q.y() = laserOdometry.pose.pose.orientation.y;
    result_q.z() = laserOdometry.pose.pose.orientation.z;
    result_t.x() = laserOdometry.pose.pose.position.x;
    result_t.y() = laserOdometry.pose.pose.position.y;
    result_t.z() = laserOdometry.pose.pose.position.z;
    Eigen::Matrix3d result_R = result_q.toRotationMatrix();

    fprintf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", result_R.row(0)[0], result_R.row(0)[1], result_R.row(0)[2], result_t.x(), result_R.row(1)[0], result_R.row(1)[1], result_R.row(1)[2], result_t.y(), result_R.row(2)[0], result_R.row(2)[1], result_R.row(2)[2], result_t.z());
    fclose(fp);
}
...
void global_optimization()
{
    while (1)
    {
        if (!loopInfoBuf.empty() && !pointCloudSurfBuf.empty() && !pointCloudEdgeBuf.empty() && !odometryBuf.empty())
        {
            ...
            FILE *fp = NULL;
            fp = fopen(Loop_RESULT_PATH.c_str(), "a");
            Eigen::Quaterniond result_q;
            Eigen::Vector3d result_t;

            result_q.w() = pose_temp.pose.orientation.w;
            result_q.x() = pose_temp.pose.orientation.x;
            result_q.y() = pose_temp.pose.orientation.y;
            result_q.z() = pose_temp.pose.orientation.z;
            result_t.x() = pose_temp.pose.position.x;
            result_t.y() = pose_temp.pose.position.y;
            result_t.z() = pose_temp.pose.position.z;
            Eigen::Matrix3d result_R = result_q.toRotationMatrix();
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh("~");
    nh.getParam("/Odo_RESULT_PATH", Odo_RESULT_PATH);
    nh.getParam("/Loop_RESULT_PATH", Loop_RESULT_PATH);
