...
PointToPointCostFunction::PointToPointCostFunction(const Eigen::Vector3d &p_, const Eigen::Vector3d &q_)
    : p(p_), q(q_) {}

bool PointToPointCostFunction::Evaluate(const double *const *parameters, double *residuals, double **jacobians) const
{
    // Extract rotation (as a quaternion) and translation from the parameters
    Eigen::Map<const Eigen::Quaterniond> rotation(parameters[0]);
    Eigen::Map<const Eigen::Vector3d> translation(parameters[0] + 4);

    // Transform point p by the rotation and translation
    Eigen::Vector3d p_transformed = rotation * p + translation;

    // Compute the difference between the transformed point and the target point q
    Eigen::Vector3d diff = p_transformed - q;

    // Compute the squared norm of the difference as the residual
    ...
    if (jacobians != NULL && jacobians[0] != NULL)
    {
        ...
        // Compute the Jacobian for the translation part
        J.block(0, 3, 1, 3) = 2 * diff.transpose();

        // Compute the Jacobian for the rotation part if needed
        if constexpr (false)
        { // Replace with actual condition if rotation Jacobian is needed
            Eigen::Matrix3d skew_diff = skew(diff);
            J.block(0, 0, 1, 3) = -2 * (skew_diff * p_transformed).transpose();
        }
    }
