...
class PointToPointCostFunction : public ceres::SizedCostFunction<1, 7>
{
public:
	PointToPointCostFunction(const Eigen::Vector3d &p_, const Eigen::Vector3d &q_);
	bool Evaluate(const double *const *parameters, double *residuals, double **jacobians) const;

	Eigen::Vector3d p; // Current point
	Eigen::Vector3d q; // Target point
};
